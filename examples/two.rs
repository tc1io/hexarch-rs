...
async fn main() -> Result<(), AppError> {
    ...
    let user_repo2 = UserRepo { _connection: "proto:mydb/foo".into()};
    ...
    api_handler_get_user2(user_repo2,get_user).await?;
...
//     Ok(())
// }
async fn api_handler_get_user2<T: UserLoader + 'static>(loader: T,use_case: GetUser<T>) -> Result<(), AppError> {
    let user_id = 42;  // Eg read input from somewhere
    let user = use_case.run(loader,user_id).await?;
    println!("User: {}",user.name);
    Ok(())
}
...
impl<T: UserLoader + 'static> GetUser<T> {
    ...
    pub fn run(self, loader: T,id: i32) -> impl std::future::Future<Output=Result<User, AppError>> + 'static {
        //Box::new(self.user_loader.load_user(id))
        loader.load_user(id)
    }
}
